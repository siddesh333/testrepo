...
- Coordination with DevOps team for AWS IAM role creation

## 🔄 Integration Workflow

This integration involves two teams working in sequence:

1. **Platform Engineer Team**:
   - Creates the Terragrunt configuration in this repository
   - Runs `terragrunt apply` to create the Datadog integration
   - Receives the **External ID** from Datadog
   - Shares the External ID with the DevOps team

2. **DevOps Team**:
   - Receives the External ID from Platform Engineers
   - Creates an IAM role in the AWS account using the [integrations-aws-iam-role](https://github.azc.ext.hp.com/runway-incubator/hpip-devex-runway-terraform-datadog/tree/master/modules/integrations-aws-iam-role) module
   - Configures trust policy with the External ID
   - Applies appropriate monitoring permissions
...
**Note**: After running `terragrunt apply`, Datadog will generate an **External ID**. This External ID is critical for the next step.

### Step 9: Share External ID with DevOps Team

After successful Terragrunt apply:

1. **Platform Engineer Team** will:
   - Capture the **External ID** generated by Datadog from the Terragrunt output
   - Share this External ID with the **DevOps team** responsible for the AWS account

2. **DevOps Team** will:
   - Use the provided External ID to create an IAM role in their AWS account
   - Apply appropriate permissions for Datadog monitoring
   - Use the AWS IAM role module: [integrations-aws-iam-role](https://github.azc.ext.hp.com/runway-incubator/hpip-devex-runway-terraform-datadog/tree/master/modules/integrations-aws-iam-role)

**Important**: The External ID is a security credential that ensures only authorized Datadog integration can assume the AWS role. Keep it secure and share it only with the designated DevOps team.

### Step 10: DevOps Team - Create IAM Role in AWS Account

The DevOps team will create the IAM role using the provided External ID:
